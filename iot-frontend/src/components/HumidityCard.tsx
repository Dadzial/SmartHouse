...
const HumidityCard: React.FC<SideBarLoginProps> = ({ darkMode }) => {
    ...
    useEffect(() => {
        const fetchHumidity = async () => {
            setError(null);
            try {
                const response = await fetch(`http://localhost:3100/weather/humidity/${cityName}`);
    ...
    const progressValue = humidity !== null
        ? ((humidity - MIN_HUMIDITY) / (MAX_HUMIDITY - MIN_HUMIDITY)) * 100
        : 0;
                    ...
                    {humidity !== null && (
                        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                            ...
                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mt: 1 }}>
                                <Typography sx={{ fontSize: 12 }}>{MIN_HUMIDITY}%</Typography>
                                <LinearProgress
                                    variant="determinate"
                                    value={progressValue}
                                    sx={{
                                ...
                                />
                                <Typography sx={{ fontSize: 12 }}>{MAX_HUMIDITY}%</Typography>
                            </Box>
                        </Box>
                    )}
                </Box>
